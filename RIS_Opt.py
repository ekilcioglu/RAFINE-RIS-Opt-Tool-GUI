...
class RIS_GUI:
    ...
    def draw_binary_poor_coverage_map(self, cm_no_ris_tensor, avg_path_gain_low_power_cells, cov_ratio):
        ...
        text_str = (
            f"$P_{{\\mathrm{{th}}}}$ = {float(self.entry_threshold.get())} dB\n"
    ...
    def show_phase_profiles(self):
        ...
        for i in range(num_positions):
            fig_pp = self.ris.phase_profile.show(mode=i)
            fig_pp.get_axes()[0].set_xlabel("Y-axis (m)")
            fig_pp.get_axes()[0].set_ylabel("Z-axis (m)")
            fig_pp.get_axes()[0].set_zlabel("Radians")
